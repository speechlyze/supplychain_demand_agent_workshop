...
export function useAgentSocket(wsUrl: string): UseAgentSocket {
  ...
  useEffect(() => {
    ...
    ws.onclose = () => { setConnected(false); setIsThinking(false); };
  ...
  const touchNode  = useCallback((id: string) =>
    setNodeActivity((p) => ({ ...p, [id]: Date.now() })), []);
  const touchEdge  = useCallback((id: string) =>
    setEdgeActivity((p) => ({ ...p, [id]: Date.now() })), []);
  const touchTable = useCallback((id: string) =>
    setTableActivity((p) => ({ ...p, [id]: Date.now() })), []);
    ...
    switch (evt.type) {
      ...
      case "token": {
        ...
        if (a === "supervisor") {
          ...
          if (aid) {
            setChat((p) => p.map((t) =>
              t.id === aid ? { ...t, content: t.content + evt.token } : t,
            ));
          }
        }
        return;
      ...
      case "final_answer": {
        ...
        if (aid) {
          setChat((p) => p.map((t) =>
            t.id === aid
              ? { ...t, content: evt.content || t.content, pending: false }
              : t,
          ));
        }
      ...
      case "error":
        setIsThinking(false);
        setChat((p) => [...p, { id: "e-" + Date.now(), role: "assistant", content: `⚠ ${evt.message}` }]);
        return;
    }
  ...
  return {
    threadId, connected, isThinking, chat, agents,
    edgeActivity, nodeActivity, tableActivity, send,
  };
}
...
export async function fetchTableRows(name: string, opts: { limit?: number; search?: string } = {}): Promise<TableRowsResponse> {
