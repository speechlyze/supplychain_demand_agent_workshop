...
interface PaletteStyle { bg: string; border: string; color: string; }
...
function ArchitectureInner({ edgeActivity, nodeActivity }: Props) {
  ...
  useEffect(() => { fetchArchitecture().then(setGraph).catch(() => null); }, []);

  useEffect(() => {
    if (graph && !fitDoneRef.current) {
      const id = setTimeout(() => {
        try { rf.fitView({ padding: 0.2 }); } catch { /* no-op */ }
  ...
  useEffect(() => {
    const stamps = [
      ...Object.values(nodeActivity),
      ...Object.values(edgeActivity),
    ];
  ...
  const activeGlow  = theme === "dark"
    ? "0 0 0 3px rgba(255, 209, 102, 0.35)"
    : "0 0 0 3px rgba(180, 83, 9, 0.30)";
  ...
  const onInit = useCallback(() => { /* fitView handled by effect */ }, []);
