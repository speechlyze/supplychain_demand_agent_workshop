...
const COLOR: Record<AgentName, { dot: string; chip: string; label: string }> = {
  supervisor:     { dot: "bg-accent-oracle", chip: "bg-accent-oracle/15 text-accent-oracle", label: "supervisor"     },
  policy_agent:   { dot: "bg-accent-skill",  chip: "bg-accent-skill/15  text-accent-skill",  label: "policy_agent"   },
  demand_analyst: { dot: "bg-accent-memory", chip: "bg-accent-memory/15 text-accent-memory", label: "demand_analyst" },
};
...
export function MemoryContext({ agents }: Props) {
  ...
  const status: "idle" | "running" | "done" =
    trace.finished ? "done" : trace.started ? "running" : "idle";
          ...
          return (
            ...
            >
              <div className="flex items-center gap-1.5">
                <span className={`w-1.5 h-1.5 rounded-full ${c.dot} ${s === "running" ? "animate-pulse" : ""}`} />
        ...
        <Section title="Status">
          <div className="text-[11px] font-mono space-y-1">
            <KV
              ...
              v={status}
              accentClass={
                status === "running" ? "text-accent-tool" :
                status === "done"    ? "text-accent-memory" :
                                       "text-text-muted"
              }
            />
            <KV k="started"  v={trace.started  ? new Date(trace.started).toLocaleTimeString()  : "—"} />
            <KV k="finished" v={trace.finished ? new Date(trace.finished).toLocaleTimeString() : "—"} />
          </div>
        </Section>
        ...
        <Section title={`Tool calls (${trace.toolCalls.length})`}>
          {trace.toolCalls.length === 0 ? (
            ...
            <div className="space-y-1.5">
              {trace.toolCalls.map((tc) => <ToolCallRow key={tc.id} tc={tc} />)}
            </div>
          )}
        </Section>
...
function ToolCallRow({ tc }: { tc: ToolCall }) {
  ...
  return (
    <div className="border border-border-subtle rounded bg-bg-panel text-[11px]">
      <button onClick={() => setOpen((o) => !o)} className="w-full px-2 py-1.5 flex items-center gap-2 text-left">
        ...
        {argsPreview && (
          <span className="text-text-muted truncate flex-1">({argsPreview})</span>
        )}
        {open
