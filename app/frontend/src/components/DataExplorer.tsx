...
export function DataExplorer({ tableActivity }: Props) {
  ...
  useEffect(() => {
    const touched = Object.entries(tableActivity)
    ...
    <div className="pane h-full">
          ...
          return (
            ...
            >
              ...
              <div className="text-[10px] text-text-muted mt-0.5">
                {t.row_count != null ? `${t.row_count.toLocaleString()} rows` : (t.exists ? "—" : "not seeded")}
              </div>
            </button>
          );
        })}
      ...
      {activeMeta && (
        <div className="px-3 py-2 border-b border-border-subtle bg-bg-panel flex items-center gap-3 shrink-0">
          <div className="text-[11px] text-text-secondary flex-1 truncate">{activeMeta.purpose}</div>
          ...
          <table className="w-full text-[11px] font-mono">
            <thead className="sticky top-0 bg-bg-panel z-10">
              <tr className="border-b border-border-subtle">
                {rows.columns.map((c) => (
                  <th key={c.name} className="text-left px-3 py-1.5 font-semibold border-r border-border-subtle whitespace-nowrap">
                    <span className="text-text-primary">{c.name}</span>
                    <span className={`ml-1.5 text-[9px] ${TYPE_COLOR[c.type] || "text-text-muted"}`}>
                      {c.type}
                    </span>
                  </th>
                ))}
...
function Cell({ value, type }: { value: unknown; type: string | undefined }) {
  ...
  return <td className={cls} title={value == null ? "null" : String(value)}>{body}</td>;
}
