...
def _ensure_vector_pool(conn) -> None:
    ...
    print(f"[bootstrap] setting vector_memory_size = {VECTOR_POOL_MB}M (scope=spfile) — restart required for it to take effect.")
    cur.execute(
        f"ALTER SYSTEM SET vector_memory_size = {VECTOR_POOL_MB}M SCOPE=spfile"
    )
    ...
    try:
        ...
        if "ORA-28009" in str(e):
            ...
            conn = oracledb.connect(
                user="sys", password=SYSTEM_PASSWORD, dsn=DSN,
