...
ask the assistant: *"How are our soccer-related items moving?"*

If the assistant uses keyword search, it scans every catalogue row for
the word **"soccer"** and returns matches. That works fine — *if* the
catalogue uses the word "soccer." Real catalogues don't:

- Brand language: *adidas Brazuca 2017 Official Match Ball*
- Sport-of-origin language: *FIFA*
- Generic-equipment language: *Match Ball*, *Cleat*
- Misclassification: a football cleat filed under *baseball & softball*
  because someone fat-fingered the category dropdown

None of those rows contain the literal word *soccer*. A keyword filter
...
ranks by cosine similarity. *"soccer merchandise demand"* lives near
*Adidas RG III Mid Football Cleat* in that space because
